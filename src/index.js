...
exports.getMaxAPYProtocol = async (vaultAddress, strategyAddress) => {
    try {
        ...
        if ((maxAPYProtocol.data.status) && ((maxAPYProtocol.data.data).length > 0)) {
            console.log("Max APY Protocol", maxAPYProtocol.data.data)
            return (maxAPYProtocol.data.data.protocolAddress)
        } else {
            return 0
        }
    } catch (error) {
...
exports.setActiveProtocol = (relayer, safeAddress, protocolAddress) => {
    try {
...
exports.callingEARN = (relayer, safeAddress, vaultAssetList, totalAssetPriceList) => {
    try {
        ...
        console.log("instruction: ", earnInstruction)
        return 'success'
    } catch (error) {
            ...
            try {
                ...
                if (vaultActiveStrategy && vaultActiveStrategy != '0x0000000000000000000000000000000000000000') {
                    ...
                    if (vaultActiveProtocol && vaultActiveProtocol == "0x0000000000000000000000000000000000000000") {
                        let protocolAddress = await exports.getMaxAPYProtocol(vault.vaultAddress, vaultActiveStrategy)
                        if (protocolAddress != 0) {
                            let activeProtocolHash = await exports.setActiveProtocol(relayer, vault.vaultAddress, protocolAddress)
                            return activeProtocolHash
                        } else {
