...
let minimumDeposit = '10000000000000000000000'; //10k Dollars
//**************************************************************************************** */
...
const earn = async (relayer, vault, amountToInvest, safeContract) => {
    try {
            ...
            try {
                ...
                if (!amountToInvest.eq(new BN('0'))) {
                    ...
                    if (vaultActiveStrategy.toLowerCase() === singleAssetStrategyAddress.toLowerCase()) {
                        /*** Calling EARN*/
                        let earnHash = await earn(relayer, vault, amountToInvest, yieldsterContractInstance);
                        if (earnHash.status){
