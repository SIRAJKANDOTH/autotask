...
// const token = "wss://mainnet.infura.io/ws/v3/af7e2e37cd6545479e7523246fbaaa08";
const token = "ws://localhost:8545";
...
const getReimbursedTxns = async (txns) => {
    ...
    const reimbursedMapping = new Map();
    let receiptStatus = await Promise.all(
        txnReceipts.map(async (element) => {
            let obj = element;
            let receipt = await web3.eth.getTransactionReceipt(element.paybackReceipt);
            let status=false;
            if (receipt) {
                status = receipt.status;
                obj.gascosts = receipt.gasUsed;
            }
            if (status === true) {
                obj.isReimbursed = true;
                reimbursedMapping.set(element.transactionHash, element.paybackReceipt)
            }
            return obj;
        })
    );
    return {reimbursedMapping,receiptStatus};
}
...
exports.handler = async function (event) {
    ...
    if ((vaults.data.status) && ((vaults.data.data).length > 0)) {
        await Promise.all((vaults.data.data).map(async (vault) => {
            ...
            let {reimbursedMapping,reimbObj} = await getReimbursedTxns(txns);

            let toBeReimbursed = txns.filter(element => {
                return (reimbursedMapping.get(element.transactionHash) === undefined)
            });

            console.log(reimbObj)

            let alreadyReimbursed = txns.filter(element => {
                return (reimbursedMapping.has(element.transactionHash))
            });
            ...
            for (let index = 0; index < toBeReimbursed.length; index++) {
                ...
                let obj = {};
                let totalTxnCost = web3.utils.toWei((element.gascosts * web3.utils.fromWei(element.gasprice)).toString()); //TO CHANGE :- get txn gas cost using eth_gettransaciton
                _assetArr = sortAssetArr(_assetArr);
            ...
            if(toBeSentTxn.length>0) {
                ...
                console.log(estimatedGas) //2292768
                let txnHash = await sendTransaction(minterInstruction, relayerAddress, safeMinter, estimatedGas);
                if (txnHash.status) {
                    let res = toBeReimbursed.map((element) => {
                        return { safeaddress: vault.vaultAddress, transactionHash: element.transactionHash, paybackReceipt: txnHash.txn }
                    });
                    let updatetxncost = await axios.patch(`${BASE_URL}/defender/updatetxncost`, res[0])
                }
            }

            let res = alreadyReimbursed.map((element) => {
                return { safeaddress: vault.vaultAddress, transactionHash: element.transactionHash, paybackReceipt: reimbursedMapping.get(element.transactionHash),status:true }
            });
            let updatetxncost = await axios.patch(`${BASE_URL}/defender/updatetxncost`, res[0])
